...
!!! tip "Converting Formats"
    Use `scripts/prep_rfdetr_data.py` to convert your existing YOLO-format dataset into COCO format for RF-DETR.
...
Open `configs/train.yaml` and uncomment the engine block you want:
...
=== ":robot: RF-DETR (Transformer, DINOv2)"
    ...
    model_type: "rfdetr"
    model_size: "m"
    optimizer_config: "configs/optimizers/rfdetr_optim.yaml"
    dataset_path: "data/rfdetr_dataset"
    batch_size: 4
    image_size: 640
    ```
...
```bash
python scripts/run_train.py --resume models/yolo/24-03-2026/weights/last.pt
```
