...
:material-file-code: **Source**: `isitec_app/performance_monitor.py`
...
End-of-session summaries are saved to `isitec_app/logs/sessions.json` (last 10 sessions). Each entry records model type, duration, FPS, average confidence, ID ratio, class counts, and the baseline hardware snapshot.
