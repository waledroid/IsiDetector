...
`models/yolo/<run>/weights/best.onnx` will produce sibling files like
`best.fp16.onnx`, `best.int8.onnx`, `best.engine` in the same directory.
This matches the existing `src/inference/export_engine.py` convention
and means the web app's auto-discovery picks them up without any config
changes.
