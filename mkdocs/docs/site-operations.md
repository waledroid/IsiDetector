...
!!! info "Branch availability"
    Most of what's documented here ships on the **`fps`** branch as of
    April 2026 — the experimental track that becomes the next `deploy`
    cut. Each feature below is independent and can be adopted or rolled
    back on its own. Where a feature has already been cherry-picked to
    `deploy` it's noted in-line; otherwise read **"on `fps`"** as the
    default.

This page covers the operator-facing workflow for a Linux site PC
running IsiDetector behind plexiglass: hands-free boot, RTSP camera
configuration, ROI cropping for low-res inference, and the standalone
`autostart.sh` helpers that turn three independent OS-level layers
into a power-button-to-running kiosk.

For initial install steps see [Getting Started](getting-started.md);
for the docker compose lifecycle see [Deployment (Docker)](deployment.md);
for NIC freeze and UDP egress diagnostics see [Network Setup](network-setup.md).
