...
:material-file-code: **Source**: `isidet/src/inference/onnx_inferencer.py`
...
Replicates the native `.pth` postprocess from `isidet/src/training/trainers/rfdetr.py:92-140`:
...
For sub-millisecond inference on Ampere+ GPUs, export to [TensorRT](tensorrt.md) `.engine` format via `isidet/src/inference/export_engine.py` and route via the `.engine` extension instead.
