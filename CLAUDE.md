...
- **X11 enforcement** — RustDesk's screen-capture pipeline doesn't work on Wayland (compositor's security model blocks the keystroke / framebuffer access). On GDM3 hosts the script edits `/etc/gdm3/custom.conf` to set `WaylandEnable=false`, backing up the original first. The change is idempotent and only flagged as "reboot needed" if the runtime session is still Wayland after the rewrite. LightDM / SDDM are skipped (they default to X11 already). Detected via `loginctl show-session ... -p Type` with a fallback to `pgrep Xwayland|Xorg`.
