...
class UDPPublisher:
    ...
    def publish(self, class_name, event_id=None):
        """Emit one UDP datagram for a line-crossing event.

        Args:
            class_name: Class label string (e.g. "carton", "polybag").
            event_id: Optional ByteTrack tracker ID. Included in the
                payload as ``"id"`` so downstream consumers (sorter
                controllers) can dedupe and trace individual objects.
                Omitted from the payload if ``None`` — backward-compatible.
                Note ``id`` is **not** sequential; use ``seq`` (always
                present) to detect lost datagrams.
...
class LiveReader:
    ...
    **File sources** (MP4, AVI): reads are paced to the video's native
    ...
    def _open_capture(self):
        ...
        can't decode a frame. This avoids on-site troubleshooting trips
        for the few cameras / firewalls where TCP is blocked.

        Non-RTSP sources (file, integer webcam index) skip the dance —
        they have no transport to choose.

        Also caps the open timeout at 5 s via
        ``CAP_PROP_OPEN_TIMEOUT_MSEC`` so a bad URL fails fast instead
        of hanging the inference thread for 30+ s.

        Returns:
            cv2.VideoCapture — caller checks ``isOpened()``.
        """
        is_rtsp = (
        ...
        # Open params: 5s open timeout (so bad URLs fail in seconds).
        # Wrap in try/except: older OpenCV (< 4.5) doesn't accept the
        # 3-arg form and raises; fall back to the bare ctor in that case.
        def _try_open():
            try:
        ...
        cap = _try_open()
        if cap.isOpened():
            # Quick read test — ffmpeg sometimes claims open() succeeded
            # but the TCP stream is dead behind a firewall.
            ok, _ = cap.read()
            if ok:
        ...
        # Restore TCP as the default for any subsequent reconnect attempt
        # — UDP only "won" because TCP failed *this time*; if the network
        # heals we want to try TCP again on the next reconnect.
        os.environ['OPENCV_FFMPEG_CAPTURE_OPTIONS'] = 'rtsp_transport;tcp'
        return cap
...
class StreamHandler:
    def __init__(self):
        ...
        # After bucket restructure this file is at webapp/isitec_app/stream_handler.py,
        ...
        #   1. settings.json (operator-edited from Settings UI; persists across restart)
        ...
        try:
            ...
            pass  # bad JSON or missing file — fall through to env defaults
    ...
    def _preload_default_onnx(self):
        try:
            ...
            # webapp/isitec_app/stream_handler.py → parent.parent.parent = repo root.
            abs_path = Path(path)
    ...
    def set_line_config(self, orientation=None, position=None, belt_direction=None):
        ...
        Re-initializes the LineZone so the new trigger anchor
        (derived from orientation × belt_direction) takes effect
    ...
    def start(self, source, model_type, weights, device=None, imgsz=None, conf_thresh=None):
        ...
        with self.lock:
            ...
            # Reads `clahe_enabled` and other future preprocess toggles.
            try:
                settings_path = Path(__file__).parent / 'settings.json'
                ui_settings = {}
                if settings_path.exists():
        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    # 320×N vs ~3 ms at 1080p). Each stage is failure-tolerant:
                    # if any process() raises, we drop that stage for the rest
                    # of the session rather than killing the loop.
                    ...
                    # One UDP datagram PER crossing — if two objects crossed
