...
document.addEventListener('DOMContentLoaded', () => {
    ...
    navBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Fetch aggregated report + session history when Analytics opens
    ...
    async function fetchStats() {
        try {
            ...
            // Update UDP footer indicator — show tracker id so operators can
            // correlate the UI line with the UDP datagram the sorter receives.
    ...
    function _fmtPeak(peakHour) {
        if (!peakHour) return '—';
        // Backend returns "YYYY-MM-DD HH:00". Strip seconds, keep full form.
        return peakHour.replace(':00$', ':00');
    }
    ...
    async function fetchReport(period) {
        try {
            ...
            const peakTxt = d.peak && d.peak.hour
                ? `${d.peak.hour} (${d.peak.events})`
                : '—';
            ...
            // Align export date pickers with the active window on first open.
            ...
            if (toEl && !toEl.value) {
                // window.to is exclusive (next day midnight) — subtract 1 day for UI
    ...
    // Period buttons (Today / Yesterday / 7d / 30d)
    document.querySelectorAll('#reportPeriodBtns .filter-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Reset date pickers so fetchReport re-aligns them to the new window.
    ...
    // CSV export — navigate so the browser triggers the download dialog.
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                function populateSelect(selectEl, models) {
                    ...
                    // Any weights that don't match a known extension fall
                    // into a catch-all group at the bottom.
        ...
        async function beginCapture() {
            try {
                ...
                img.onload = () => {
                    ...
                    // Match canvas backing-store to native snapshot resolution
                    ...
                    // Compute scale from CSS-displayed pixels back to native frame coords.
                    // The canvas backing store is at native res, but it's CSS-stretched
                    // to fit the .video-wrapper. We use getBoundingClientRect at click time.
                    savedClickHandler = videoCanvas.onclick;
                    videoCanvas.onclick = (e) => {
                        const r = videoCanvas.getBoundingClientRect();
                        const cssX = e.clientX - r.left;
                        const cssY = e.clientY - r.top;
                        const nx = Math.round(cssX * (videoCanvas.width / r.width));
                        const ny = Math.round(cssY * (videoCanvas.height / r.height));
                        pendingPoints.push([nx, ny]);

                        // Draw a numbered green marker at the click position
                        ...
                        if (pendingPoints.length < 4) {
                            ...
                            const x1 = Math.min(...xs), x2 = Math.max(...xs);
                            const y1 = Math.min(...ys), y2 = Math.max(...ys);
                            // Draw the computed bounding rectangle
