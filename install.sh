...
#   3. Clones the IsiDetector deploy branch into $INSTALL_DIR.
...
#     into $INSTALL_DIR/isidet/models/ or $INSTALL_DIR/isidet/runs/ after install.
#     Typical: scp -r isidet/models/yolo/<date>/openvino user@prod-pc:~/logistic/isidet/...
#   - Does not set up auto-start on boot (see the systemd / cron one-liner
#     emitted at the bottom of run_start.sh's output).
#
...
#   curl -fsSLo install.sh https://raw.githubusercontent.com/waledroid/IsiDetector/deploy/install.sh
#   chmod +x install.sh
#   ./install.sh
#
...
#
# This script is self-contained: it works both when run from a clone (./install.sh)
# and when piped from curl (curl ... | bash). Nothing it does depends on the
...
if [ -d "$INSTALL_DIR" ]; then
    ...
    info "Cloning (shallow, ${BRANCH} branch only)..."
    git clone --branch "${BRANCH}" --depth 1 "${REPO_URL}" "${INSTALL_DIR}"
    cd "$INSTALL_DIR"
    success "Cloned to ${INSTALL_DIR}"
fi

# Make runtime scripts executable. compress.sh only exists on main; ignore
# its absence on deploy. The trailing `|| true` keeps that case quiet.
chmod +x run_start.sh up.sh net.sh install.sh 2>/dev/null || true
chmod +x compress.sh 2>/dev/null || true
...
echo -e "         ${CYAN}${INSTALL_DIR}/isidet/models/yolo/<date>/openvino/${NC}   (.xml + .bin)"
echo -e "         or"
echo -e "         ${CYAN}${INSTALL_DIR}/isidet/runs/segment/models/yolo/<date>/weights/${NC}"
echo -e "       From your dev machine, for example:"
echo -e "         ${CYAN}scp -r isidet/models/yolo/<date>/openvino USER@\$(hostname -I|awk '{print\$1}'):${INSTALL_DIR}/isidet/models/yolo/<date>/${NC}"
if [ "$do_bootstrap" = false ]; then
